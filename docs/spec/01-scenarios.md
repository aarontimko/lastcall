...
**D23 — Uncommitted work and its accepted hunk survive `checkout -b` (B2 with an override).** Setup: first sight on `main`; `f1` has two hunks; accept hunk 1. Expect: `[f1]` with hunk 2 only. Action: `git checkout -b feat/w` (git carries the uncommitted `f1`). Expect: a copy, the override carried, pile `[f1]` with exactly hunk 2, the notice `switched main → feat/w: first time here, seen state carried from main; 1 file pending`. Action: `git checkout main`. Expect: `[f1]`, hunk 2, the return notice. *(H, E. PROPOSED with Amendment v1.12, ratified by merging the Phase 11 PR.)*
