...
**Gate — worker half closed 2026-09-01 at `d5a6e38` (kickoff: `docs/spec/90-phase1-kickoff.md`; orchestrator re-ran every item personally):**
- [x] `just build && just lint && just test-unit` green locally on macOS: 99 unit tests (engine 84, testkit 15, bin 0); `ci.yml` runs exactly those targets plus `just test-integration-herdr` on `ubuntu-latest` and `macos-latest`. CI-run link: attached to the PR after push.
- [x] Client state machine driven by the mock: 29 `client_*` tests in `crates/lastcall-engine/src/herdr/client.rs` (in-memory transport, paused time) plus 7 transport tests over the socket mock.
...
- [ ] **[sponsor]** Hello-herdr in a real herdr session: recipe at `docs/dev/hello-herdr.md`; automated proxy `just probe-hello` shows both transitions. Transcript to be pasted in the PR by the sponsor.
...
- **2026-09-01 — Phase 1 rulings** (presented at close-out; each `[LEANING]` becomes `[DECIDED]` when the sponsor merges the Phase 1 PR or replies otherwise):
  ...
  - **Phase 2 unit-test floor: 99** (engine 84, testkit 15, bin 0), recorded here; ratchets only upward.
