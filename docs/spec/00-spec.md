...
- [ ] Performance baseline recorded (entry obligation of the Phase 9 performance gate; amendment of 2026-09-02, §10): `just bench` runs the four scale scenarios at the ruled sizes — a parent of 100 clones carrying 4,000 pending rows; one 100,000-line diff opened in the TUI; a 1,000-file burst write under watch; a 50,000-file drop into one root — and records scan wall time, git spawn count, first-frame and edit-to-screen latency, and peak RSS in `docs/dev/bench.md` with the machine noted. No targets yet (Phase 9 sets them against these numbers). The 50,000-file scenario also establishes the cutoff behaviour: above a per-root row cap the engine stops materialising rows and the pile carries a notice the TUI shows (`<N> pending files; showing the first <cap>` — cap and exact text ruled at the Phase 4 kickoff), never an unbounded scan.
