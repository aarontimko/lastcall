...
- Wire protocol is a single monotone u32; v0.8.2 = **protocol 21**. `ping` result: `{"type":"pong","version":"0.8.2","protocol":21,"capabilities":{...}|null}`.
...
**Gate:**
- [ ] `just build && just lint && just test-unit` green in CI on both OSes (link to run). Split interpretation, ratified with the G0 stamp: the worker cannot push, so the worker's evidence is the same commands green locally plus workflow files that invoke exactly them; the CI-run link is attached by the orchestrator after the sponsor pushes, before merge.
- [ ] Client state machine driven by the mock server: bootstrap, both envelope shapes, subscribe-failure-closes-connection, per-pane sub open/close on pane lifecycle (test names).
- [ ] Config round-trip tests: every v1 key parsed, override env vars honored, unknown keys rejected (test names).
- [ ] Isolated-herdr integration test (spawn pinned herdr per §5.10) passes in CI on Linux.
- [ ] **[sponsor]** Hello-herdr, run inside a real herdr session, prints a `working→done` transition and a `done→idle` flip caused by focusing the tab (transcript pasted in PR).
- [ ] Allowlist file present; orchestrator confirms from the worker transcript that no permission prompt fired (orchestrator-verified, not worker-reported).
...
**Gate:**
- [ ] Full integration suite for the git state machine passes: every §7.2 git scenario has a named test (list in PR), including **commit-then-review** (agent commits an unreviewed edit → still pending), **merge/pull of upstream** (files land in the `upstream` group, not individual rows; conflict resolutions stay individual), **agent push** (no change to the pile), **pull --rebase / rebase -i** (content-preserving rewrites re-flag nothing), **branch-switch-and-back** (over-show then self-clear; overrides intact), symlink, mode-only, CRLF, sparse-checkout, in-progress rebase/merge, unstaged rename.
- [ ] Property test (accept/recompute invariant) green over ≥1000 cases (seed + count in report).
- [ ] Crash-recovery test: kill -9 mid-accept, restart, recomputed pending identical to pre-kill oracle (test name + assertion).
- [ ] HEAD-movement detection: a commit with no other file activity triggers re-inspection within the poll interval (test).
- [ ] `status --json` output for a scripted multi-repo scenario matches a committed golden file.
- [ ] Unit-test floor recorded (this number seeds the ratchet).
...
(none yet; log entries take the form: v1.1, date, phase N, change, reason it was unknowable at design time)
