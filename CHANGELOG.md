...
- Two lastcall processes that first scan the same repository at the same moment no longer
  risk one of them failing that scan with "No such file or directory". Both wrote one
  shared temp file beside the private index; each write now has its own.
