...
/// Amendment v1.12 through a real terminal: the reported bug, and its absence.
///
/// An unattended run cuts a branch from `HEAD`, commits on it, and the user accepts the
/// work as it comes. Before this phase, checking the start branch back out showed every
/// accepted file again as a deletion, because one repository had one seen record and
/// `main`'s working tree has none of that content. Here the branch the run cut first-sights
/// its own record, the accept lands on that record alone, and checking `main` back out
/// loads `main`'s parked record: the run's file is simply absent, no deletion row appears,
/// and returning to the run's branch finds its pile still empty.
#[test]
fn pty_branch_switch() {
    let _serial = SERIAL.lock().unwrap_or_else(|e| e.into_inner());
    let fx = Fixture::build();
    let Some(mut pty) = fx.spawn_tui(&bin()) else {
        return;
    ...
    // (1) the run: a branch off alpha's HEAD with a generated name, and a commit on it.
    let mut alpha = fx.repo("alpha");
    alpha.checkout_b("feat/pty").expect("the run's branch");
    pty.wait_for(OVERLOADED, |s| s.contents().contains("feat/pty · 3 files"))
        .unwrap_or_else(|e| {
            panic!(
                "the branch never reached the nav: {e}\n{}",
                pty.screen_text()
            )
        });
    assert!(
        status_is_prefixed(&pty, "switched main → feat/pty (same commit)"),
        "the branch was cut at HEAD, so R8 keeps today's wording:\n{}",
        pty.screen_text()
    );
    // `commit_files` stages everything, so the fixture's two uncommitted edits ride along;
    // that is the shape the fold has to deal with, three paths and not one.
    alpha
        .commit_files(&[("gen.txt", "generated by the run\n")], "the run's commit")
        .expect("the run's commit lands");
    pty.wait_for(OVERLOADED, |s| {
        let t = s.contents();
        t.contains("feat/pty · 4 files") && t.contains("gen.txt")
    })
    .unwrap_or_else(|e| panic!("the run's file never arrived: {e}\n{}", pty.screen_text()));

    // (2) the user accepts the repository. Four files is under the confirm threshold, so
    // `A` on the repository row folds it at once (`a` there refuses and names `A`).
    select_until(&mut pty, "alpha  feat/pty · 4 files");
    let t = Instant::now();
    pty.send(b"A").expect("A");
    pty.wait_for(OVERLOADED, |s| {
        s.contents().contains("nothing pending in alpha")
    ...
    note(&format!(
        "PTY branch switch: alpha folded after {:.3?}",
        t.elapsed()
    ));

    // (3) the return, which is the whole point: `main`'s own record comes back off the
    // shelf. Its three unreviewed commits are pending again because that is what `main`
    // had not seen, and the run's file is simply not in `main`'s tree: it does not come
    // back as a deletion, which is what one record for the whole repository used to do.
    let t = Instant::now();
    alpha.checkout("main").expect("back to main");
    pty.wait_for(OVERLOADED, |s| s.contents().contains("main · 3 files"))
        .unwrap_or_else(|e| panic!("the return: {e}\n{}", pty.screen_text()));
    note(&format!(
        "PTY branch switch: back on main after {:.3?}",
        t.elapsed()
    ));
    let text = pty.screen_text();
    assert!(
        !text.contains("gen.txt"),
        "the reported bug: the accepted file must not come back as a deletion:\n{text}"
    );
    assert!(
        !text.contains("deleted"),
        "and no deletion row of any kind:\n{text}"
    );
    assert!(
        status_is_prefixed(
            &pty,
            "switched feat/pty → main: 3 files differ from seen state"
        ),
        "the load's notice, R8:\n{text}"
    );

    // (4) and back again: `feat/pty`'s record was parked whole, so it comes back whole.
    alpha
        .checkout("feat/pty")
        .expect("back to the run's branch");
    pty.wait_for(OVERLOADED, |s| s.contents().contains("feat/pty · 0 files"))
        .unwrap_or_else(|e| panic!("the second arrival: {e}\n{}", pty.screen_text()));
    let text = pty.screen_text();
    assert!(!text.contains("gen.txt"), "still nothing pending:\n{text}");
    let ledger = fx.ledger("alpha");
    assert_eq!(ledger["seen_branch"].as_str(), Some("feat/pty"));
    assert_eq!(
        ledger["branches"]
            .as_object()
            ...
            .unwrap_or_default(),
        vec!["main".to_string()],
        "one parked record, main's: {ledger}"
    );
...
/// [`status_is`] for a notice long enough that the status line may have to shorten it: the
/// row must begin with as much of `text` as fits beside the ` · <age>` suffix. A head
/// notice is the one status text whose length is the engine's and not the view's.
fn status_is_prefixed(pty: &PtyTui, text: &str) -> bool {
    pty.screen(|s| {
        let (_, cols) = s.size();
        s.rows(0, cols).last().is_some_and(|r| {
            let row = r.trim_end();
            let Some((head, age)) = row.rsplit_once(" · ") else {
                return false;
            };
            !age.is_empty() && !age.contains(' ') && text.starts_with(head) && head.len() > 20
        })
    })
}
